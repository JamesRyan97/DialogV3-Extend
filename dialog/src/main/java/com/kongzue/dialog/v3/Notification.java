...
public class Notification {
    ...
    public static Notification build(Context context, String message) {
        synchronized (Notification.class) {
            ...
            notification.log("Loading message notification:" + notification.toString());
    ...
    public void showNotification() {
        log("Start message notification -> " + toString());
        isShow = true;
        ...
        public void show(final Context context, final View view) {
            ...
            try {
                ...
                if (mTN != null) {
                    ...
                    try {
                        //At present, there is no way. The TN of the new version of Android Toast must have IBinder to show, and IBinder must obtain the WindowManagerImpl of the mWM instantiated object in TN, which is almost the same
    ...
    //Catch the BadTokenException of Toast before 8.0, Google fixed this problem in Android 8.0 code submission (By @Dovar66[https://github.com/Dovar66/DToast])
    ...
    public Notification setStyle(DialogSettings.STYLE style) {
        this.style = style;
        if (isShow) {
            error("You must use the build(...) method to create setStyle(...) to modify the notification theme or style.");
        }
        return this;
    }
